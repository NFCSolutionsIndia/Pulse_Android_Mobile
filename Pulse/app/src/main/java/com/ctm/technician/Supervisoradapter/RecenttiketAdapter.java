...
public class RecenttiketAdapter extends RecyclerView.Adapter<RecenttiketAdapter.MyViewHolder> {
    Context context;
    private CommonSharePrefrences comShare;
    private List<SupPMTechnicianData> jobsList;
    private ArrayList<SupPMTechnicianData> filterList;


    public RecenttiketAdapter(Context activity, List<SupPMTechnicianData> listOfOrdersWithServices) {
        this.context = activity;
        this.comShare = comShare;
        jobsList = listOfOrdersWithServices;

        this.filterList = new ArrayList<SupPMTechnicianData>();
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        ...
        if (jobsList.get(position).getPMTicketData().getTicketType().equals("PM")) {
            ...
            if (jobsList.get(position).getPMTicketData().getPriority().equals("High")) {
        ...
        holder.sitename.setText("#" + String.valueOf(jobsList.get(position).getPMTicketData().getSiteUniqueId() + "," +String.valueOf(jobsList.get(position).getPMTicketData().getSiteName())));
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            ...
            for (SupPMTechnicianData retailer : filterList) {
                if (retailer.getPMTicketData().getSiteUniqueId().startsWith(charText) || retailer.getPMTicketData().getSiteName().toUpperCase().contains(charText)) {
